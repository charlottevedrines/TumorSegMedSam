#%% import packages
import platform
print("Inside pre_grey_rgb2D.py Running on machine:", platform.node())
print("Inside pre_grey_rgb2D.py Python version:", platform.python_version())
...
with open("output_log.txt", "a") as f:
    print("pre_grey_rgb2D.py ", file=f)
...
def process(gt_name: str, image_name: str):
    ...
    if np.sum(gt_data) > 1:  # exclude tiny objects WAS 80 BEFORE
...
# COMMENTED THIS        assert np.sum(gt_data) > 80, "ground truth should have more than 80 pixels"
